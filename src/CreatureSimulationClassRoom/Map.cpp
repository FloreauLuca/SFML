#include <CreatureSimulationClassRoom/Map.h>
#include <iostream>
...
bool Map::AddGameObjectAt(std::unique_ptr<GameObject> gameObject, int positionX, int positionY) {
	...
	{
		cellsForUpdate.push_back(gameObject.get());
...
void Map::Update() {
	...
	cellsForUpdate.remove_if([](GameObject * gameObject) {return gameObject->IsObjectDestroyed(); });
